...
#define EXAMPLE_MQTT_SERVER_HOST "sems.ses-tech.vn"
#endif
...
static void publish_message(void *ctx)
{
    ...
    static   char*  message   = {"message from2 board"};

    uint16_t k = strlen (message) ;
    char mes2 [50] ;
    for (int i = 0 ; i <k ; i++ ){
    	mes2 [i] = i+3 ;
    }
    ...
    int index = * (int*)  (ctx) ;
    mes2[index] = message[index] + index ;
    mqtt_publish(mqtt_client, TOP_PIC, mes2 , strlen(message), 0 , 0, mqtt_message_published_cb, TOP_PIC );
}

/*!
 * @brief Application thread.
 */
int arrIndex [5] ={0 , 1, 2, 3 ,4} ;
static void app_thread(void *arg)
{
    ...
    for (i = 0; i < 5;)
    {
        ...
        if (connected)
        {
             err = tcpip_callback(publish_message, &arrIndex [i]);
        	// publish_message (NULL) ; // will be called in tcpip thread
            if (err != ERR_OK)
            {
